                                   ...
                                   int *transfer_indices, int k)
{
  ...
  Path* heavypath_root = do_heavy_decomposition(alt_tree->node0, k);
  ...
  for(int i=0; i < ref_tree->nb_taxa-1; i++) //last leaf node is already removed. leaves are sorted.
  {
    u = ref_leaves[i];                     //Start with a leaf
    if (u==NULL) fprintf(stderr, "u is Null: current i is : %d", i);
...
// Node* find_matched(Path* w, unsigned int spl)
// {
//   if (w->num_child_paths>0){
//     // leaf of PT 
//     // fprintf(stderr, "child heavy path");
//     if (split_match(w->node->subtree_split, matched_bipar_rapid, spl)) return w->node;
//     Node* a;
//     for (int i=0; i<w->num_child_paths; i++){
//       a = find_matched(w->child_heavypaths[i], spl);
//       if (a!=NULL) return a;
//     }
//     return a;
//   }else if (is_HPT_leaf(w)){
//     // fprintf(stderr, "HPT leaf");
//     if (split_match(w->node->subtree_split, matched_bipar_rapid, spl)) return w->node;
//     // fprintf(stderr, "processed\n");
//     return NULL;
//   }else{
//     // fprintf(stderr, "internal");
//     Node* a = find_matched(w->left, spl);
//     // fprintf(stderr, "left finished");
//     if (a==NULL) a = find_matched(w->right, spl);
//     // fprintf(stderr, "right finished\n");
//     return a;
//   }
// }
...
void add_heavy_path(Node *u, Tree *alt_tree, bool use_HPT)
{
  ...
  int* order_array = alt_tree->node0->path->order_array;
  ...
  {                                      //not seen a heavier sibling
    ...
    if(use_HPT)
    {
      compare_two_sorted_ascending(hpt_root->d_min_path, hpt_root->d_min_subtree, 
                                                order_array, u->k, u->k, u->k);
      set_int_array_from_order_2(hpt_root->d_min_path, hpt_root->d_min_subtree, u->ti_min, order_array, u->k);
      set_node_array_from_order_2(hpt_root->d_min_path_node, hpt_root->d_min_subtree_node, u->min_node, order_array, u->k);

      compare_two_sorted_descending(hpt_root->d_max_path, hpt_root->d_max_subtree, 
                                                order_array, u->k, u->k, u->k);                                       
      set_int_array_from_order_2(hpt_root->d_max_path, hpt_root->d_max_subtree, u->ti_max, order_array, u->k);
      // if (hpt_root->d_max_subtree[0] > 10000) fprintf(stderr, "d_max_subtree: %d\n", hpt_root->d_max_subtree[0]); -> it's always d_max_subtree
      // if (hpt_root->d_max_path[0] > 10000) fprintf(stderr, "d_max_path: %d\n", hpt_root->d_max_path[0]);
      set_node_array_from_order_2(hpt_root->d_max_path_node, hpt_root->d_max_subtree_node, u->max_node, order_array, u->k);


    }
    else
    {
      u->ti_min[0] = alt_tree->node0->d_min;
      u->ti_max[0] = alt_tree->node0->d_max;
    }
    DB_CALL(0, fprintf(stderr, "++++++++ TI: %i %i\n", u->ti_min[0], u->ti_max[0]));

    // if (split_match(u->subtree_split, bipar_rapid, spl)) {
    //   fprintf(stderr, "u id: %d\n", u->id);
    //   fprintf(stderr, "Split matched.");
    //   Node* matched = find_matched(hpt_root, spl);
    //   if (matched==NULL) fprintf(stderr, "was null.\n");
    //   fprintf(stderr, "Node ID %d\n", matched->id);
    //   fprintf(stderr, "d_min_path %d\n", matched->path->d_min_path[0]);
    //   fprintf(stderr, "diff_path %d\n", matched->path->diff_path);

    //   set_path_to_root_HPT(matched->path, *matched->path->path_to_root_p);
    //   Path** path = *matched->path->path_to_root_p;
    //   int pathlen = path[0]->total_depth + 1; 

    //   int d = path[0]->d_min_path[0];
    //   fprintf(stderr, "d at this point: %d\n", d);
    //   fprintf(stderr, "path0 id: %d\n", path[0]->id);
    //   fprintf(stderr, "path0 id: %d\n", matched->path->id);

    //   int diff_subtree = 0;
    //   for (int i=1; i<pathlen; i++) {
    //     fprintf(stderr, "Path ID of path %d: %d\n", i, path[i]->id);
    //     if (diff_subtree) d+= path[i-1]->diff_subtree;
    //     else{
    //       d += path[i-1]->diff_path;
    //       if (path[i]->num_child_paths>0) diff_subtree = 1;
    //     }
    //     fprintf(stderr, "d at this point: %d\n", d);
    //   }
...
void check_after_reset(Path* root_path){
  Path* w = root_path;
  bool good = true;

  good = good && (w->diff_path==0);
  good = good && (w->diff_subtree==0);

  if (is_HPT_leaf(w)){
    good = good && (w->d_max_path[0] == 1);
    good = good && (w->d_min_path[0] == 1);
  }else if (w->num_child_paths>0){
    for (int i=0; i<w->num_child_paths; i++) check_after_reset(w->child_heavypaths[i]);
    good = (good && (w->d_min_path[0] == w->node->subtreesize));
    good = (good && (w->d_max_path[0] == w->node->subtreesize));
  }else{
    check_after_reset(w->left);
    check_after_reset(w->right);
  }

  if (!good) fprintf(stderr, "BADBADBAD\n");
}
...
{
  // Path* path_u = get_HPT_root(u->other);
  while(u)                               //Have not visited the root and have
  {                                      //not seen a heavier sibling
      //Add the leaves from the light subtree:
...
void set_edge_TI(Node *u, int n)
{
  if (u!= NULL){
    ...
    {
      DB_TRACE(0, "min %i max %i ", u->ti_min[0], n - u->ti_max[0]);
      // DB_CALL(0, print_node(u));
      // fprintf(stderr, "min %i max %i \n", u->ti_min[0], n - u->ti_max[0]);
      u->br[0]->transfer_index = min(u->ti_min[0], n - u->ti_max[0]);
      // if (min(u->ti_min[0], n - u->ti_max[0]) < 0) fprintf(stderr, "min %i max %i \n", u->ti_min[0], n - u->ti_max[0]);;
    }
  }
}
