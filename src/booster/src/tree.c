...
Node* new_node(const char* name, Tree* t, int degree) {
	...
	nn->ti_max = (int*)malloc(sizeof(int));
	nn->ti_max[0] = 0;
	nn->ti_min = (int*)malloc(sizeof(int));
	nn->ti_min[0] = INT_MAX;
	// nn->ti_max = 0;
	// nn->ti_min = INT_MAX;
...
Tree* new_tree(const char* name) {
	...
	t->node0 = newNode(t, 1);	/* this first node _is_ a leaf */
...
Node* copy_node_rapidTI(Node* old) {
  ...
  new->ti_min = (int*)malloc(new->k*sizeof(int));
  new->ti_max = (int*)malloc(new->k*sizeof(int));
  for(int i=0; i<new->k;i++){
	new->ti_min[i] = old->ti_min[i];
	new->ti_max[i] = old->ti_max[i];
  }

  new->min_node = (Node**)calloc(new->k, sizeof(Node*));
  new->max_node = (Node**)calloc(new->k, sizeof(Node*));
  for(int i=0; i<new->k;i++){
	new->min_node[i] = old->min_node[i];
	new->max_node[i] = old->max_node[i];
  }
...
Tree* prune_branches(Tree* t, int* node_ids, int n_prune){
	for (int i=0; i<n_prune; i++){
		// Prune t->a_nodes[i] from t.
		Node* node_to_prune = t->a_nodes[node_ids[i]];
		if (node_to_prune==t->node0){
			fprintf(stderr, "Root specified for pruning is not allowed. Exiting.\n");
			exit(1);
		}
		if (node_to_prune->nneigh==1){
			fprintf(stderr, "Leaf specified for pruning is not allowed. Exiting.\n");
			exit(1);
		}
		/* node_to_prune is an internal_node*/
		Node* parent = node_to_prune->neigh[0];
		/* remove node_to_reroot from its parent*/
		int current_ind = -1;
		for (int l=0; l<parent->nneigh; l++){
			if (parent->neigh[l] == node_to_prune) {
				current_ind = l;
				/* Set neigh and br to NULL*/
				parent->neigh[l] = NULL;
				parent->br[l] = NULL;
				break;
			}
		}
		for (int l=current_ind; l<parent->nneigh-1; l++){
			parent->neigh[l] = parent->neigh[l+1];
			parent->br[l] = parent->br[l+1];
		}
		parent->nneigh--; // reduce neighbor count;

		/* Connect parent to children*/
		for (int l=1; l<node_to_prune->nneigh; l++){
			Node* child = node_to_prune->neigh[l];
			if (parent->nneigh_space <= parent->nneigh){
				/* Reallocation of neigh and br*/
				int new_space = 2 * parent->nneigh_space;
				parent->neigh = realloc(parent->neigh, sizeof(Node*) * new_space);
				parent->br = realloc(parent->br, sizeof(Edge*) * new_space);
				parent->nneigh_space = new_space;
			}
			parent->neigh[parent->nneigh] = child;
			parent->br[parent->nneigh] = node_to_prune->br[l];
			node_to_prune->br[l]->left = parent;
			parent->nneigh ++;

			child->neigh[0] = parent;
			// child->br[0] already have parent as left.
		}

		/* WE don't free nodes and edges in order for free_tree to work properly */
		// free_edge(node_to_prune->br[0]);
		// free_node(node_to_prune);
	}
}
...
Node* graft_new_node_on_branch(Edge* target_edge, Tree* tree, double ratio_from_left, double new_edge_length, char* node_name) {
	...
	if(target_edge == NULL) {
		...
		Node* second_node = newNode(tree, 1); /* will be the right node, also a leaf */
...
// Helper function to find the path from the root to the target leaf.
// It stores the path in the `path` array and returns the length of the path.
int find_path_from_root(Tree* tree, Node* target, Node** path) {
	Node* root = tree->node0;
	Node** reverse_path = malloc(tree->nb_nodes * sizeof(Node*));
	reverse_path[0] = target;
	int index = 1;
	Node* current = target;

	while(current!=root){
		current = current->neigh[0];
		reverse_path[index++] = current;
	}

	for (int i=0; i<index; i++){
		path[i] = reverse_path[index-1-i];
	}
	free(reverse_path);
    return index;
}

// Function to remove a leaf from the tree by adjusting its parent's neighbors.
void remove_root_leaf(Tree* tree, Node* leaf) {
    Node* parent = leaf->neigh[0];  // The parent of the leaf is always neigh[0]

	// fprintf(stderr, "SHOULD BE TRUE %d\n", tree->node0==parent);
	// for (int i=0; i<tree->nb_nodes; i++){
	// 	if (tree->a_nodes[i] != NULL) fprintf(stderr, "Node nneigh%d\n", tree->a_nodes[i]->nneigh );
	// }

    // Find the index of the leaf in the parent's neighbors
    int leaf_index = -1;
    for (int i = 0; i < parent->nneigh; ++i) {
        if (parent->neigh[i] == leaf) {
            leaf_index = i;
            break;
        }
    }

	// if (leaf_index !=0 )fprintf(stderr, "something wrong!\n");

    if (leaf_index != -1) {
        // Remove the leaf from the parent's neighbors by shifting the neighbors
        for (int i = leaf_index; i < parent->nneigh - 1; ++i) {
            parent->neigh[i] = parent->neigh[i + 1];
			parent->br[i] = parent->br[i+1];
        }
		parent->nneigh--;  // Decrement the parent's nneigh count
		// if (parent->neigh[parent->nneigh]==NULL) fprintf(stderr, "Already NULL\n");
		parent->neigh[parent->nneigh] = NULL; // Set last pointer to NULL
		parent->br[parent->nneigh] = NULL;
    }


	tree->a_edges[leaf->br[0]->id] = NULL; // We just set to NULL;
	tree->a_nodes[leaf->id] = NULL; // We just set to NULL;
	tree->leaves->a[leaf->leaf_id] =  NULL; // We just set to NULL pointer.

	// // free the edge connecting parent to the leaf node.
	// free_edge(leaf->br[0]);

	// Instead of freeing the leaf edge, we store it at the outgroup_edge
	tree->outgroup_edge = leaf->br[0];

	// Instead of freeing the leaf node, we store it at the outgroup_node
	tree->outgroup_node = leaf;
    // free_node(leaf); 
...
void addback_root_leaf(Tree* tree){
	tree->outgroup_node->nneigh = 1;
	tree->outgroup_node->neigh[0] = tree->node0;
	tree->a_nodes[tree->outgroup_node->id] = tree->outgroup_node;
	tree->leaves->a[tree->outgroup_node->leaf_id] = tree->outgroup_node;

	Node* root = tree->node0; 
	if (root->nneigh_space <= root->nneigh){
		/* Reallocation of neigh and br*/
		int new_space = root->nneigh_space+1;
		root->neigh = realloc(root->neigh, sizeof(Node*) * new_space);
		root->br = realloc(root->br, sizeof(Edge*) * new_space);
		root->nneigh_space = new_space;
	}
	root->neigh[tree->node0->nneigh] = tree->outgroup_node;
	root->br[tree->node0->nneigh] = tree->outgroup_edge;
	root->nneigh += 1;

	tree->a_edges[tree->outgroup_edge->id] = tree->outgroup_edge;
	tree->outgroup_edge->left = tree->node0;
	tree->outgroup_edge->right = tree->outgroup_node;

	tree->outgroup_node = NULL;
	tree->outgroup_edge = NULL;
}

// Function to reroot the tree at the given leaf node and remove the leaf
void reroot_and_remove_leaf(Tree* tree, Node* leaf) {
	// We remove the leaf node and edge connecting leaf to the parent.
	// However, we don't edit tree->nb_edges and tree->nb_nodes, and 
	// we just set tree->a_nodes[leaf->id] and tree->a_edges[leaf->br[0]->id] to NULL (for efficiency).
	// We also don't remove taxon name from namespace.
    if (!leaf || leaf->nneigh == 0) {
        // The node must be a leaf to reroot
		fprintf(stderr, "Invalid node passed for rerooting\n. Skipping rerooting process.");
        return;
    }

	// fprintf(stderr, "leaf taxon to be removed: %s\n", leaf->name);

    // Allocate memory to store the path from the current root to the leaf
    Node** path = (Node**)malloc(sizeof(Node*) * tree->nb_nodes);

	// fprintf(stderr, "root==leaf %d\n", tree->node0 == leaf);

    int path_length = find_path_from_root(tree, leaf, path);
	// fprintf(stderr, "path length: %d\n", path_length);

    if (path_length == 0) {
        // Leaf not found in the tree, invalid reroot request
        free(path);
        return;
    }

    // Reverse the parent-child relationship along the path. From root to leaf.
    for (int i = 0; i < path_length - 1; ++i) {
        Node* current = path[i];
        Node* next = path[i + 1];

        // current->neigh[0] is the current parent, we need to make `next` the new parent
        // and adjust the other neighbors to ensure the tree remains connected.

        // Find the index of `next` in the `current->neigh` array
        int next_index = -1;
        for (int j = 0; j < current->nneigh; ++j) {
            if (current->neigh[j] == next) {
                next_index = j;
                break;
            }
        }

        // If next is not found in neigh, something went wrong (should not happen)
        if (next_index == -1) {
			fprintf(stderr, "somethign wrong!\n");
            continue;
        }

        // Swap `current->neigh[0]` (the parent) with `current->neigh[next_index]` (the child)
		// Reverse Edge connecting current and next
		Edge* edge = current->br[next_index];
		// if (edge->left != current) fprintf(stderr, "something wrong!\n");
		edge->left = next;
		edge->right = current;
		// Set current's parent as next
		Node* temp = current->neigh[0];
        current->neigh[0] = next;
		// Set the neighbor position that "next" was placed to the 0-th neighbor (parent, or else in case of root)/
        current->neigh[next_index] = temp;
		current->br[next_index] = current->br[0]; // Branch from current to original parent.
		current->br[0] = edge; //Edge from next to current (reveresed).
    }

	// We have to remove root node if the original tree is rooted.
	if (tree->node0->nneigh==2){
		// fprintf(stderr, "Rooted from first\n");
		Node* par = path[1]; // the child node of the root that is connected to the leaf node.
		Node* chil = tree->node0->neigh[1]; // The other child

		int par_root_index;

		for (int j=0; j<par->nneigh; j++){
			if (par->neigh[j] == tree->node0){
				par_root_index = j;
				break;
			}
		}
		...
		Edge* par_to_root = par->br[par_root_index];
		// free the edge from root to chil
		tree->a_edges[chil->br[0]->id] = NULL;
		free_edge(chil->br[0]);
		par_to_root->right = chil;
		par->neigh[par_root_index] = chil;
		chil->br[0] = par_to_root;
		chil->neigh[0] = par;
		// free the original root node.
		tree->a_nodes[tree->node0->id] = NULL;
		free_node(tree->node0);
	}

    // Set the new root of the tree to the parent of the leaf
    tree->node0 = leaf->neigh[0];

    // Remove the leaf from the tree after rerooting
    remove_root_leaf(tree, leaf);
...
Edge* connect_to_father(Node* son, Node* father, Tree* current_tree) {
	...
	assert (son->nneigh == 0); // Make sure that 0th neigh is going to be the parent.
...
Tree* parse_nh_string(char* in_str, int k) {
	...
	t->outgroup_node = NULL;
	t->outgroup_edge = NULL;
	...
	level = 0;
	last_tok = parse_iter(t, in_str, &i, in_length, &level, k);
	//printf("Root ??? %d\n",t->node0);
	if(level != 0) {
...
Node* newNode(Tree *t, int k){
	...
	node->ti_max = (int*)malloc(k*sizeof(int));
	node->ti_min = (int*)malloc(k*sizeof(int));
	// Initialize all values.
	for(int i=0; i < k; i++){
		node->ti_max[i] = 0;
		node->ti_min[i] = INT_MAX;
	}

	node->min_node = (Node**)calloc(k, sizeof(Node*));
	node->max_node = (Node**)calloc(k, sizeof(Node*));

	node->lightleaves = NULL;
	node->subtree_split = NULL;

	return node;
}


/* Change k and reallocate memory accordingly. */
void realloc_nodes_k(Tree *t, int k){
	for (int i=0; i<t->nb_nodes; i++){
		if(t->a_nodes[i]!=NULL){
			Node* node = t->a_nodes[i];
			int old_size = node->k;
			node->k = k;
			node->ti_max = (int*)realloc(node->ti_max, k*sizeof(int));
			node->ti_min = (int*)realloc(node->ti_min, k*sizeof(int));
			for(int i=old_size; i < k; i++){
				node->ti_max[i] = 0;
				node->ti_min[i] = INT_MAX;
			}
			node->min_node = (Node**)realloc(node->min_node, k*sizeof(Node*));
			node->max_node = (Node**)realloc(node->max_node, k*sizeof(Node*));
			for(int i=old_size; i<k; i++){
				node->min_node[i] = NULL;
				node->max_node[i] = NULL;
			}
		}
	}
}
...
Tree *complete_parse_nh(char* big_string, char*** taxname_lookup_table,
	...
	/* trick: iff taxname_lookup_table is NULL, we set it according to the tree read, otherwise we use it as the reference taxname lookup table */
 	Tree* mytree = parse_nh_string(big_string, k);
	mytree->id = TREE_COUNT++;
	if (treelist !=NULL) treelist[mytree->id] = mytree;
	mytree->hashtable = M1M2hashtable;
	mytree->leaves = allocateNA(mytree->nb_taxa);
	mytree->bipar_count = add_hash_count;
  ...
  if (HASH_INTEGERS1 == NULL){
	// initialize HASH_INTEGERS1 and HASH_INTEGERS2.
	srand(123456789); /*seed of the random generator*/
	HASH_INTEGERS1 = malloc(sizeof(unsigned int)*mytree->nb_taxa);
	HASH_INTEGERS2 = malloc(sizeof(unsigned int)*mytree->nb_taxa);
	Floyd_random_gen(mytree->nb_taxa, M1, HASH_INTEGERS1);
	Floyd_random_gen(mytree->nb_taxa, M2, HASH_INTEGERS2); // Could take a lot of time...?
  }
...
Edge* find_edge_from_hash_recur(Tree* tree, Node* current, unsigned int h1, unsigned int h2){
	/* Add search logic. It does not work with root hash.*/
	Edge* edge = NULL;
	int start_index=1;
	if (current == tree->node0) start_index = 0;
	for (int i=start_index; i<current->nneigh; i++){
		edge = find_edge_from_hash_recur(tree, current->neigh[i], h1, h2);
		if (edge!=NULL) break;
	}
	if (edge==NULL && current->h1 == h1 && current->h2 == h2) {
		if (tree->node0 == current){
			fprintf(stderr, "it corresponds to the root hash, but it is not supported.");
			exit(1);
		}
		edge = current->br[0]; // Edge pending current;
	}
	return edge;
}

Edge* retrieve_edge_from_hash(Tree* tree, unsigned int h1, unsigned int h2){
	/* We assume that tree is alreday parsed in tree.*/
	Edge* edge = find_edge_from_hash_recur(tree, tree->node0, h1, h2);
	return edge;
}

Edge* retrieve_edge(unsigned int h1, unsigned int h2, M1M2Hashtable* hashtable, Tree** treelist){
	IntSet* hash_loc = find_element_M1M2hashtable(hashtable, h1, h2);
	int tree_id = hash_loc->treeid;
	return retrieve_edge_from_hash(treelist[tree_id], h1, h2);
}
...
void post_order_traversal_recur(Node* current, Node* origin, Edge *e, Tree* tree, void (*func)(Node*, Node*, Edge*, Tree*)) {
	...
	if (current == NULL) fprintf(stderr, "CURRENT NULL\n");
	// fprintf(stderr, "nneigh %d\n", current->nneigh);
	int i, n = current->nneigh;
	// fprintf(stderr, "preorder nneigh %d\n", n);
	...
	// fprintf(stderr, "postorder-func to nneigh %d data\n", n);
	// fprintf(stderr, "is root %d\n", current == tree->node0);
	// fprintf(stderr, "origin NULL %d\n", origin==NULL);
	// fprintf(stderr, "node0 nneigh %d\n", tree->node0->nneigh);
...
void update_node_depths_pre_doer(Node* target, Node* orig, Edge *e, Tree* t) {
  ...
  if(target->nneigh == 1)           //leaf
  {
    e->topo_depth = target->subtreesize = 1;
		e->transfer_index = -1;
    addNodeNA(t->leaves, target); // Add leaf node to t->leaves
  }
  target->depth = orig->depth+1;
} /* end of update_node_heights_pre_doer */


void update_only_node_depths_pre_doer(Node* target, Node* orig, Edge *e, Tree* t){
  if(!orig) { //root
    target->depth = 0;
    return;
  }
...
void free_node(Node* node) {
  ...
  if (node->ti_max) free(node->ti_max);
  if (node->ti_min) free(node->ti_min);
  if (node->min_node) free(node->min_node);
  if (node->max_node) free(node->max_node);
  if (node->subtree_split) free(node->subtree_split);
	free(node->neigh);
	free(node->br);
	if(node->subtree_split) free(node->subtree_split);
	free(node);
...
void free_tree(Tree* tree) {
	...
	if(tree->outgroup_node) free_node(tree->outgroup_node);
	if(tree->outgroup_edge) free_edge(tree->outgroup_edge);
...
void prepare_rapid_TI(Tree* mytree) {
	...
	for(int i=0; i<mytree->nb_taxa; ++i) mytree->leaves->a[i]->leaf_id = i; // set to leaf_id sorted index.

	// for (int i=0; i<mytree->nb_taxa; i++) if (i!=mytree->leaves->a[i]->leaf_id) fprintf(stderr, "%d is not equal to %d\n", i, mytree->leaves->a[i]->leaf_id);

	// Reroot
	reroot_and_remove_leaf(mytree, mytree->leaves->a[mytree->nb_taxa -1]);

	// Recompute Node Depths
	pre_order_traversal(mytree, &update_only_node_depths_pre_doer);
...
void setup_heavy_light_subtrees(Node *u)
{
  ...
  {
	int tmp=0;
	if (u->heavychild == u) tmp=1;
	if (u->neigh[i]->subtreesize==1) tmp=2;
...
void add_leaves_in_subtree(Node *u, NodeArray *leafarray)
{
  ...
  {
    for (int i=1; i<u->nneigh; i++){
		add_leaves_in_subtree(u->neigh[i], leafarray);
	}
  }
}
...
void prepare_rapid_TI_doer(Node* target, Node* orig, Edge *e, Tree* t) {
  ...
  {
	// if (t->bipar_count==0){
	// 	/* It is reference tree, so have the split representation explicitly */
	// 	target->subtree_split = (split) calloc(split_length(t->nb_taxa), sizeof(unsigned int));
	// 	set_bit(target->subtree_split, target->leaf_id);
	// }
    // e->topo_depth = target->subtreesize = 1;
	// e->transfer_index = -1;

	/*hash computation*/
	target->h1 = HASH_INTEGERS1[target->leaf_id];
	target->h2 = HASH_INTEGERS2[target->leaf_id];
	// fprintf(stderr, "h1 and h2: %u, %u\n", target->h1, target->h2);
	/* add to hashtable*/
	if (target->h1 >= M1) fprintf(stderr, "leaf hash too big!\n");

	add_M1M2hashtable(t->hashtable, target->h1, target->h2, t->id, 1-t->bipar_count, 1, target->id);
  }
  else                              //internal node
  {
    target->subtreesize = 0;
	unsigned int leng = split_length(t->nb_taxa);
	// if (t->bipar_count==0) target->subtree_split = (split) calloc(leng, sizeof(unsigned int));
	target->h1 = 0;
	target->h2 = 0;
    for(int i=0; i < target->nneigh; i++){
		if(target->neigh[i]!=orig){
			target->subtreesize += target->neigh[i]->subtreesize;
			// if (t->bipar_count==0) bitwise_or(target->subtree_split, target->neigh[i]->subtree_split, target->subtree_split, leng); // for ref tree
			/*hash computation*/
			target->h1 += target->neigh[i]->h1;
			target->h2 += target->neigh[i]->h2;
		}
	}
	...
	int is_root = (target == t->node0);

	add_M1M2hashtable(t->hashtable, target->h1, target->h2, t->id, 1-t->bipar_count, is_root, target->id);

    if(!is_root)          //set topo_depth for edges above nodes
		{
...
int argmin(int i1, int i2)
{
  if(i1 > i2)
    return 1;
  return 0; // Return 0 when two integers are equal.
}
...
void print_node_TI(const Node* n) {
  ...
  fprintf(stderr,
          ...
          n->id, name, n->subtreesize, n->depth, n->ti_min[0], n->ti_max[0]);
}
...
void rec_print_ref_tree_dot(Node* n, FILE* f) {
  if(n->nneigh == 1)
    fprintf(f, "  %i [label=\"%i (%s): %i, %i\"];\n",
            n->id, n->id, n->name, n->ti_min[0], n->ti_max[0]);
  else
    fprintf(f, "  %i [label=\"%i: %i, %i\"];\n",
            n->id, n->id, n->ti_min[0], n->ti_max[0]);
...
/* Helper function to grow the buffer if needed */
void ensure_capacity(char** buffer, size_t* capacity, size_t new_len) {
...
/* Function to append formatted data to the buffer */
void append_to_buffer(char** buffer, size_t* capacity, size_t* offset, const char* format, ...) {
    va_list args;
    va_start(args, format);
    
    // Determine how much space is required
    size_t remaining = *capacity - *offset;
    int required = vsnprintf(*buffer + *offset, remaining, format, args);

    va_end(args);

    // If the buffer wasn't big enough, grow it and try again
    if (required >= remaining) {
        ensure_capacity(buffer, capacity, *offset + required);
        va_start(args, format);
...
/* the following function writes the subtree having root "node" and not including "parent". */
void write_subtree_to_buffer(Node* current, Node* parent, Edge* e, char** buffer, size_t* capacity, size_t* offset) {
    int i, n = current->nneigh;

    if (n == 1) {
        /* terminal node */
        append_to_buffer(buffer, capacity, offset, "%s:%g", (current->name ? current->name : ""), e->brlen); /* distance to father */
    } else {
        append_to_buffer(buffer, capacity, offset, "(");
        /* we have to write (n-1) subtrees in total. The last print is not followed by a comma */
        int nc = 0;
        for (i = 0; i < n; i++) {
            if (current->neigh[i] != parent) {
                ...
                write_subtree_to_buffer(current->neigh[i], current, current->br[i], buffer, capacity, offset); /* a son */
                nc++;
            }
        }
        append_to_buffer(buffer, capacity, offset, ")");
        ...
        if (e->has_branch_support) {
            append_to_buffer(buffer, capacity, offset, "%g:%g", e->branch_support, e->brlen); /* distance to father */
        } else {
            append_to_buffer(buffer, capacity, offset, "%s:%g", (current->name ? current->name : ""), e->brlen); /* distance to father */
        }
    }
} /* end write_subtree_to_buffer */


/* writing the tree from the current position in the buffer */
void write_nh_tree_to_buffer(Tree* tree, char** buffer, size_t* capacity, size_t* offset, bool newline) {
    ...
    for (i = 0; i < n; i++) {
        ...
        write_subtree_to_buffer(node->neigh[i], node, node->br[i], buffer, capacity, offset); /* a son */
    }
    append_to_buffer(buffer, capacity, offset, ")");

    if (node->name) {
        append_to_buffer(buffer, capacity, offset, "%s", node->name);
    }

    /* Terminate with a semicolon and optionally a newline */
    append_to_buffer(buffer, capacity, offset, ";");
    if (newline) {
        append_to_buffer(buffer, capacity, offset, "\n");
    }
