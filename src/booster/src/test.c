...
int test_classical_bootstrap(){
  ...
  Tree* ref_tree = complete_parse_nh(ref_tree_string, &taxname_lookup_table, false, 1); /* sets taxname_lookup_table en passant */
  Tree* boot_tree = complete_parse_nh(boot_tree_string, &taxname_lookup_table, false, 1); /* sets taxname_lookup_table en passant */
...
int test_transfer_1(){
  ...
  swap_tree = complete_parse_nh(swap_tree_string, &taxname_lookup_table, false, 1); /* sets taxname_lookup_table en passant */
  for (i = 0; i < m; i++) {
...
//((a:1,b:1,c:1):1,(d:1,e:1,f:1):1,((g:1,h:1,i:1):1,(j:1,k:1,l:1):1,(m:1,n:1,o:1):1):1);"; 
  ...
  for(r=0;r<100;r++){
    swap_tree = complete_parse_nh(ref_tree_string, &taxname_lookup_table, false, 1); /* sets taxname_lookup_table en passant */
    for (i = 0; i < m; i++) {
      min_dist[i] = n; /* initialization to the nb of taxa */
    }
...
int test_transfer_3(){
  ...
  swap_tree = complete_parse_nh(swap_tree2_string, &taxname_lookup_table, false, 1); /* sets taxname_lookup_table en passant */
  for (i = 0; i < m; i++) {
    min_dist[i] = n; /* initialization to the nb of taxa */
  }
  min_num_moved = 5;
...
int test_remove_taxon(){
  ...
  ref_tree = complete_parse_nh(boot4_tree_string, &taxname_lookup_table, false, 1); /* sets taxname_lookup_table en passant */
  ...
  ref_tree = complete_parse_nh(boot3_tree_string, &taxname_lookup_table, false, 1); /* sets taxname_lookup_table en passant */
  tax_id = get_tax_id_from_tax_name("a", taxname_lookup_table, ref_tree->nb_taxa);
  remove_taxon(tax_id,ref_tree);
  if(ref_tree->nb_nodes != 6){
    fprintf(stderr,"Test remove taxon: error - The number of nodes is %d, and should be 6\n",ref_tree->nb_nodes);
 ...
 // write_nh_tree(ref_tree,stdout);
  ...
  ref_tree = complete_parse_nh(ref_tree_string, &taxname_lookup_table, false, 1); /* sets taxname_lookup_table en passant */
